...
wide_data <- clean_data%>%
  filter(sample!="11660_tp24_Repeat", treatmentarm!= "DP 2 years", timepoint_num %in% c(8, 24, 52))%>%
  mutate(anymalar=if_else(total_n_malaria_12==0, 0, 1))%>%
  filter(timepoint_num==52)%>%
  pivot_wider(names_from = c(targetName, timepoint), values_from = conc, id_cols = c("id", "treatmentarm","anymalar"))
