...
read_full <- function(path_to_directory){
  ...
  # manual cluster merge, first fixing some CD4+ CM / Treg business in som100 then boiling it down to 34 clusters
  ...
  merging_table1 <- read.csv("/home/flobuntu/PhD/cytof/vac69a/reprocessed/reprocessed_relabeled_comped/T_cells_only/merging_tables/merging_table_sept_2020.csv", header=T, stringsAsFactors = F)

  merged_sce <- CATALYST::mergeClusters(sce, k = "som100", table = meta_45_mod_table, id = "mod_meta45")
  merged_sce <- CATALYST::mergeClusters(merged_sce, k = "mod_meta45", table = merging_table1, id = "flo_merge")
