...
flo_umap <- function (df, color_by, facet_by = NULL){
  ...
  plt <- ggplot2::ggplot(data, ggplot2::aes(x = UMAP1, y = UMAP2,
                                            color = marker)
                         )+
    ggplot2::geom_point(shape = ".") +
    viridis::scale_color_viridis(option="B")+
    UMAP_theme + ggplot2::facet_wrap(facet_by) + ggplot2::ggtitle(color_by) +
    ggplot2::theme(strip.text = facet_title)
}
