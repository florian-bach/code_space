...
```{r load libs, include=FALSE, eval=TRUE}

library(CATALYST)
library(flowCore)
library(wiebkeTOF)

data("vac69a_single_cell_experiment")
sce <- vac69a_single_cell_experiment
```
...
Much of what I'm writing here is based on this [fantastic publication](https://f1000research.com/articles/6-748) by the authors of CATALYST. I highly recommend you read it at some point as it not only covers & explains a lot of code which is about follow, but also goes a bit into some of the theoretical considerations of high-dimensional cytometry analysis. For the purposes of this tutorial we're going to re-analyse the VAC69a CyTOF data.
...
devtools::install_github("florian-bach/code_space/my_packages/wiebkeTOF")
```
...
For the purposes of this tutorial, you don't actually need to run any of the following chunk, but once you are working with your own data obviously you'll have to.
...
Some aspects of high-dimensional cytometry analysis (especially dimensionality reduction) can take a long time to run and eat up your RAM. If you just want to quickly test / optimise something, it can be useful to downsample fcs files so that instead of 100,000 cells per file you only have 1,000, for example. To do this we can do the following:
...
# I've included two functions in WiebkeTOF that can downsample, one that retains a percentage of cells and one that retains a fixed number of cells from each file. 

# Because it's random, it'll give slightly different results each time, but this can be fixed (literally) by setting the seed prior to running, this will make the "random" number generator behave predictably
set.seed(1234)

# the function randomly selects 50% of each fcs file
smaller_vac69a <- wiebkeTOF::proportional.downsample(vac69a, 0.5)
...
even_smaller_vac69a <- wiebkeTOF::hard.downsample(vac69a, 5000)

# the above line is exactly what I ran to make a small version of the vac69a dataset, which you can load like this, no fcs files needed!
...
I recommend listing all markers like this and simply commmenting out the markers you don't want to use for clustering. Don't use markers that were used to pregate data (e.g. CD45, CD3) and don't use markers where you're unsure about the quality of the staining. I ordered these in the way that I want them to be displayed in heatmaps *etc* with lineage & activation markers on the left, memory markers on the right, and everything else in between, loosely grouped as I thought made sense at the time.
...
#You may want to save this list somewhere as a txt or csv file, it will come in handy later, but it's not absolutely required. if you would like to do this just uncomment these lines

# write.table(refined_markers, "refined_markers.txt", sep = "\t", row.names = FALSE, col.names = FALSE)

# it can now be read in like this
# refined_markers <- scan("refined_markers.txt", what = "character" )
```


Like many clustering methods, FlowSOM is not deterministic, which is to that there isn't one correct solution, rather than many plausible ones. This means that different runs will yield slightly different results, as each clustering is initialised with an element of randomness. This randomness (the seed) can be fixed in order to have reproducible results when re-running the analysis.
FlowSOM wants several arguments: the `SingleCellExperiment`, which markers it should use to cluster, them number of clusters to use (take the square root and pass each integer number to `xdim` and `ydim`) and maximum number of metaclusters. **NB** the way the metaclustering works is that if e.g. `maxK=50` the results with 49, 48, 47... *etc.* are saved as well and can be inspected. So set the number at slightly larger than what you think it should be and compare a few different versions so see what looks best. You can also manually merge clusters (which we'll cover later). This is how we run flowSOM and make a quick heatmap to show the results. Later, we'll cover how these heatmaps can be beautified. 

We will display the clustering results in a heatmap where rows are clusters and columns are markers. You may (or not) wish to hierarchically cluster rows, which will group phenotypically similar clusters together. For making merging tables it's easier to leave them in numerical order, however. `bars = TRUE` draws a wee bargraph on the right indicating the size of each cluster. The "features" argument takes a vector (e.g. `c(CD4, CD8)`) and defines what markers will be shown. these could also include markers that weren't part of the clustering if you want.


```{r clustering, eval=FALSE}

set.seed(1234); sce <- CATALYST::cluster(sce, features = refined_markers, xdim = 10, ydim = 10, maxK = 50)
...
```{r phenotype plot, echo=FALSE, fig.height = 8, fig.width = 9}

set.seed(1234); sce <- CATALYST::cluster(sce, features = refined_markers, xdim = 10, ydim = 10, maxK = 30)

cluster_phenotype_heatmap <- plotExprHeatmap(x = sce, by = "cluster", row_clust = TRUE, col_clust = FALSE, k = "meta30", bars = TRUE, features = refined_markers)
