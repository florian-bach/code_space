...
line_plot_data <- select(biochem_data,  volunteer, timepoint, bilirubin, alt, alkphos, albumin, potassium, sodium)

long_line_plot_data <- line_plot_data %>%
  filter(timepoint %in% c("Baseline", "C7 am", "C14 am", "Diagnosis", "T1", "T6", "C90")) %>%
  gather(Analyte, Concentration, c(bilirubin, alt, alkphos, albumin, sodium, potassium))
...
haemoglobin_data <- haem_data %>%
  select(volunteer, timepoint, "Concentration"=haemaglobin) %>%
  mutate(Analyte="Haemoglobin")

haemoglobin_data$timepoint <- stringr::str_replace_all(haemoglobin_data$timepoint, biochem_timepoint_replacement)

haemoglobin_data <- filter(haemoglobin_data, timepoint %in% c("Baseline", "C7 am", "C14 am", "Diagnosis", "T1", "T6", "C90"))


haemoglobin_data <- na.omit(haemoglobin_data)

long_line_plot_data <- rbind(long_line_plot_data[long_line_plot_data$Analyte!="Haemoglobin", ], haemoglobin_data)

long_line_plot_data$Analyte <- gsub("Alt", "ALT", long_line_plot_data$Analyte ) 
...
ggsave("~/PhD/figures_for_thesis/chapter_1/1_biochem_line_plot.png", biochem_line_plot, width=7, height=4)
...
(all_ae_stack <- ggplot(adverse_events,  aes(x=factor(timepoint, levels=timepoint_levels), y=ae_count/6, fill=factor(severity, levels=rev(1:3))))+
    ...
    xlab("Timepoint")+
    ggtitle("All Adverse Events Through Time")+
    geom_vline(xintercept = 23.5)+
    scale_y_continuous(limits = c(0,8), breaks = seq(0, 8, by=2))+
    theme_minimal()+
    fig1_theme+
    theme(axis.text.x = element_text(hjust=1, angle=45, size=7),
...
ggsave("~/PhD/figures_for_thesis/chapter_1/1_all_adverse_events_stacked.png", all_ae_stack, height=4, width=6)
...
ggsave("~/PhD/figures_for_thesis/chapter_1/1_fever_curves.png", fever_curves, width=4, height=3)
...
haemoglobin_data <-  filter(supp, Cell == "Haemoglobin")
...
data3 <- read.csv("~/PhD/plasma/vac69a/Vivax_plasma_analytes2_no_inequalities.csv", header=T, stringsAsFactors = F)
...
long_data3$analyte <- gsub(".pg.ml.", "", long_data3$analyte, fixed = T)
long_data3$analyte <- gsub(".", "", long_data3$analyte, fixed = T)

long_data3$analyte <- gsub("IFNy", "IFNγ", long_data3$analyte)
long_data3$analyte <- gsub("TNFa", "TNFα", long_data3$analyte)
long_data3$analyte <- gsub("IFNb", "IFNβ", long_data3$analyte)
...
all_legendplex_plot <- ggplot(long_data3, aes(x=factor(timepoint, levels=c("Baseline", "Diagnosis", "T6", "C45")), y=concentration, color=Volunteer, group=Volunteer))+
  geom_line(aes(color=Volunteer), size=0.7)+
  geom_point(fill="white", stroke=0.7, shape=21, size=0.7)+
...
sig_legend_data <- subset(sig_legend_data, sig_legend_data$Volunteer %in% c("v02", "v03", "v05", "v06", "v07"))

sig_legend_data$analyte <- factor(sig_legend_data$analyte, levels=sig_analytes)

sig_glm_plot <- ggplot(sig_legend_data, aes(x=factor(timepoint, levels=c("Baseline", "Diagnosis", "T6", "C45")), y=concentration, color=Volunteer, group=Volunteer))+
  geom_line(aes(color=Volunteer), size=0.9)+
  geom_point(fill="white", stroke=1, shape=21, size=0.9)+
  scale_y_continuous(labels=scales::comma)+
