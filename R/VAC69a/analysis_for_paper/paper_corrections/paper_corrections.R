...
# redo colours of GO term circle plots ####
...
dod_data$hist_color <- ifelse(dod_data$`%Genes Cluster #1`> 65, "#db0066" , dod_data$hist_color)
# dod_data$hist_color <- ifelse(dod_data$hist_color != "#db0085"& dod_data$hist_color != "#fec200", "lightgrey", dod_data$Cluster_Difference)
  ...
  dod_dot_plot <- ggplot(dod_data, aes(x=`%Genes Cluster #2`, y=`%Genes Cluster #1`))+
    ...
    scale_y_continuous(breaks=seq(0,100,by=10), limits = c(-5, 105), expand=c(0,0))+
  ...
  pdf("~/PhD/manuscripts/vac69a/jci_corrections/final_figures/vivax_falciparum_dod_GO.pdf", height = 3, width=3.9, useDingbats = TRUE)
  ...
  t6_data$hist_color <- ifelse(t6_data$`%Genes Cluster #1`> 65, "#db0066" , t6_data$hist_color)
  ...
  pdf("~/PhD/manuscripts/vac69a/jci_corrections/final_figures/vivax_falciparum_t6_GO.pdf", height = 3, width=3.9, useDingbats = TRUE)
  vivax_falciparum_t6
  dev.off()
