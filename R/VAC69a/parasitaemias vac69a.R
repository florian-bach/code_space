...
data <- read.csv("~/PhD/clinical_data/vac69a/parasitaemia/vac69a_parasitaemia.csv", header=T)
...
parasitaemias$Timepoint <- gsub("C.1", "Baseline", parasitaemias$Timepoint) 
...
vac69a_plot <- ggplot(data=parasitaemias[!is.na(parasitaemias$Genomes),], aes(x=factor(Timepoint, levels = unique(mixedsort(parasitaemias$Timepoint))), y=Genomes, group=factor(Volunteer)))+
  ...
  scale_x_discrete(limits=c("Baseline", paste("D", seq(6, 17), sep='')))+
  parasitaemia_theme

#ggsave ("/Users/s1249052/PhD/oxford/vac69/parasitaemias_vac69.png", height = 8, width=10)
ggsave("~/PhD/clinical_data/vac69a/figures/vac69a_parasitaemia.png", vac69a_plot, height = 8, width=10)


dods_plot <- ggplot(dods, aes(x=vol, y=paras, fill=vol))+
...
ggsave("~/PhD/clinical_data/vac69a/figures/vac69a_parasitaemia_at_dod.png", dods_plot, height = 8, width=10)
