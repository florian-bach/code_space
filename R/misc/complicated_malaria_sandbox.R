...
promote_data <- promote1 %>%
  ...
  mutate(qPCRparsdens=NA)

## mic drop ####
mic_drop <-  haven::read_dta("~/Library/CloudStorage/Box-Box/MIC_DroP IPTc Study/Data/MICDroP Data/MICDROP all visit database through November 30th 2025.dta")
...
mic_drop_data <- mic_drop %>%
  ...
  mutate(treatmentarm=mic_drop_key$treatmentarm[match(as.numeric(id), mic_drop_key$id)],
         ...
         treatmentarm=case_match(treatmentarm,
                                 ...
                                 2~"DP 1 year",
                                 3~"DP 2 years"))%>%
  select(all_of(meta_cols), qPCRparsdens)
...
person_year_df <- duration_df%>%
  ungroup()%>%
  summarise("number of person years of observation"=as.numeric(sum(duration))/365)
...
monthly_categories <- combo_data %>%
  ...
  mutate(has_hyper = ifelse(any(qPCRparsdens>100000|pardens>100000), "hyperparasitemia", "normal"))%>%
  filter(
...
long_monthly_categories <- monthly_categories%>%
  tidyr::pivot_wider(names_from = outcome, values_fill = 0,values_from = order_of_outcome,
                     id_cols = c("id", "date", "treatmentarm", "flo_age_in_wks", "flo_age_in_months", "age_quarter", "age_semi", "four_week_increment", "mstatus", "pardens", "qPCRparsdens", "mom_rx", "gender"))%>%
...
malaria_model_para <- lme4::glmer(bino_complicated ~ n_any_malaria + I(n_any_malaria^2)+ log10(pardens+0.001) + mom_rx +  (1|id), family = "binomial", data=all_malaria)
...
malaria_model_age_gender_para <- lme4::glmer(bino_complicated ~ n_any_malaria + I(n_any_malaria^2) + log10(pardens+0.001) + flo_age_in_months+ gender+  mom_rx + (1|id), family = "binomial", data=all_malaria)
...
para_model_para <- lme4::glmer(bino_complicated ~ n_any_para + I(n_any_para^2) + log10(pardens+0.001)+ mom_rx + (1|id),  family = "binomial", data=all_para)
...
para_model_age_gender_para <- lme4::glmer(bino_complicated ~ n_any_para + I(n_any_para^2) + log10(pardens+0.001)+flo_age_in_months + gender +  mom_rx + (1|id),  family = "binomial", data=all_para)
...
malaria_model_para <- lme4::glmer(bino_complicated ~ n_any_malaria + I(n_any_malaria^2)+ log10(pardens+0.001) + mom_rx +  (1|id), family = "binomial", data=placebo_malaria)
...
malaria_model_age_gender_para <- lme4::glmer(bino_complicated ~ n_any_malaria + I(n_any_malaria^2) + log10(pardens+0.001) + flo_age_in_months+ gender+  mom_rx + (1|id), family = "binomial", data=placebo_malaria)
AIC(malaria_null_model, malaria_linear_model, malaria_model, malaria_model_para, malaria_model_age, malaria_model_age_gender, malaria_model_age_gender_para)
...
para_model_para <- lme4::glmer(bino_complicated ~ n_any_para + I(n_any_para^2) + log10(pardens+0.001)+ mom_rx + (1|id),  family = "binomial", data=placebo_para)
...
para_model_age_gender_para <- lme4::glmer(bino_complicated ~ n_any_para + I(n_any_para^2) + log10(pardens+0.001)+flo_age_in_months + gender +  mom_rx + (1|id),  family = "binomial", data=placebo_para)
AIC(para_null_model, para_linear_model, para_model, para_model_para, para_model_age, para_model_age_gender, para_model_age_gender_para)
...
all_para_symp_summary <- all_para%>%
  # filter(treatmentarm=="No DP" | treatmentarm=="DP 1 year"&flo_age_in_wks>56 | treatmentarm=="DP 2 years"&flo_age_in_wks>108)%>%
