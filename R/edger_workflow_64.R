...
# combine, get rid of duplicate rows (cluster and metacluster)
...
# add list to list of dataframes as new column; the simplify argument prevents conversion of every column
# to a list
...
individual_from_all <- ldply(all_degs, rbind)
...
# store what's clusters matter for what timepoint in variables
clusters_base_c8 <- cut_off %>%
  dplyr::filter(cut_off$Timepoint=="deg_base_c8") %>%
  select(Cluster, Fold_Change)

clusters_base_dod <- clusters_base_c8 <- cut_off %>%
  dplyr::filter(cut_off$Timepoint=="deg_base_dod") %>%
  select(Cluster, Fold_Change)

clusters_base_dod6 <- clusters_base_c8 <- cut_off %>%
  dplyr::filter(cut_off$Timepoint=="deg_base_dod6") %>%
  select(Cluster, Fold_Change)

clusters_dod_dod6 <- clusters_base_c8 <- cut_off %>%
  dplyr::filter(cut_off$Timepoint=="deg_dod_dod6") %>%
  select(Cluster, Fold_Change)
#list_of_degs <- all_degs
...
medians_base_c8 <- deg_medians_aggregate %>%
  dplyr::filter(ClusterID %in% as.numeric(clusters_base_c8$Cluster))%>%
  mutate(Comparison = "base_c8") %>%
  mutate(Fold_Change = clusters_base_c8$Fold_Change)

medians_base_dod <- deg_medians_aggregate %>%
  dplyr::filter(ClusterID %in% as.numeric(clusters_base_dod$Cluster))  %>%
  mutate(Comparison = "base_dod") %>%
  mutate(Fold_Change = clusters_base_dod$Fold_Change)

medians_base_dod6 <- deg_medians_aggregate %>%
  dplyr::filter(ClusterID %in% as.numeric(clusters_base_dod6$Cluster))  %>%
  mutate(Comparison = "base_dod6") %>%
  mutate(Fold_Change = clusters_base_dod6$Fold_Change)

medians_dod_dod6 <- deg_medians_aggregate %>%
  dplyr::filter(ClusterID %in% as.numeric(clusters_dod_dod6$Cluster))  %>%
...
deg_medians_aggregate <- rbind(medians_base_c8, medians_base_dod, medians_base_dod6, medians_dod_dod6)
...
for(i in unique(long_deg_medians_all$Comparison)){
  ...
  specific_levels <- unique(sub_set[order(sub_set$Fold_Change, decreasing = TRUE),"ClusterID"][c(seq(from=1, to=nrow(sub_set), by=2))])
...
238ad1d5687c3e09e3dbd3f5f85d142918c1375c
} 



<<<<<<< HEAD
ggsave("sandbox.pdf", grid.arrange(comparison_base_c8, comparison_base_dod, comparison_base_dod6, comparison_dod_dod6, ncol=3, nrow=2, layout_matrix = rbind(c(1,2,3),
=======
...
abun_clusters_02 <- short[c(as.numeric(clusters_base_c8$Cluster)),c(2,1)]
abun_clusters_02$ClusterID <- rownames(abun_clusters_02)
abun_clusters_02$Comparison <- "base_c8"
abun_clusters_02$Fold_Change <- clusters_base_c8$Fold_Change
colnames(abun_clusters_02)[1:2] <- c("post", "pre")

abun_clusters_03 <- short[c(as.numeric(clusters_base_dod$Cluster)),c(4,1)]
abun_clusters_03$ClusterID <- rownames(abun_clusters_03)
abun_clusters_03$Comparison <- "base_dod"
abun_clusters_03$Fold_Change <- clusters_base_dod$Fold_Change
colnames(abun_clusters_03)[1:2] <- c("post", "pre")

abun_clusters_05 <- short[c(as.numeric(clusters_base_dod6$Cluster)),c(5,1)]
abun_clusters_05$ClusterID <- rownames(abun_clusters_05)
abun_clusters_05$Comparison <- "base_dod6"
abun_clusters_05$Fold_Change <- clusters_base_dod6$Fold_Change
colnames(abun_clusters_05)[1:2] <- c("post", "pre")

abun_clusters_06 <- short[c(as.numeric(clusters_dod_dod6$Cluster)),c(5,4)]
abun_clusters_06$ClusterID <- rownames(abun_clusters_06)
abun_clusters_06$Comparison <- "dod_dod6"
abun_clusters_06$Fold_Change <- clusters_dod_dod6$Fold_Change
colnames(abun_clusters_06)[1:2] <- c("post", "pre")
=======
...
<<<<<<< HEAD
long_abun_clusters$Frequency <- long_abun_clusters$Count/nrow(flo_set@frames[[files_list[24]]]@exprs)
...
for(i in unique(long_abun_clusters$Comparison)){
  ...
  specific_levels <- sub_set[order(sub_set$Fold_Change, decreasing = TRUE),"ClusterID"][c(seq(from=1, to=nrow(sub_set), by=2))]
  
  assign(paste(i,"_bar", sep=''), 
    ggplot(data = sub_set,
         aes(x = factor(ClusterID, levels=specific_levels),
              y = Frequency,
              fill = factor(Timepoint, levels=c("pre", "post"))
              )
             )+
=======
...
# > specific_levels
# [1] "38" "61" "37" "29" "51" "46" "23" "28" "30" "57" "33" "19" "26" "10" "2"  "11" "18" "3" 
...
long_individual_from_all <- gather(individual_from_all, Cluster, Fold_Change, colnames(individual_from_all)[6:10] )


ggplot(data = dplyr::filter(individual_from_all, Timepoint=="deg_base_dod6"), aes(x=factor(Cluster), y=Fold_Change, fill=factor(Timepoint)))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_brewer(palette="Paired")+
  xlab("Cluster ID")+
  scale_y_log10()+
...
ggplot(data = long_abun_clusters, aes(x=factor(ClusterID), y=Count, group=Timepoint, fill=Comparison))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_brewer(palette="Paired")+
  xlab("Cluster ID")+
  scale_y_log10()+
