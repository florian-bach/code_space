...
```{r, fig.cap=c("Quadratic linear regression model for complicated malaria", "Linear regression between age and complicated malaria risk")}
...
ggplot(complicated_data, aes(x=temp, y=parsdens, color=complicatedmalaria))+
  #geom_point()+
  geom_density2d()+
  geom_vline(xintercept = 37.8, linetype="dashed")+
  scale_y_log10()+
...
The shape of the data is much better approximated by a quadratic model, both the linear and quadratic coefficients for n_infection are significant. Model error is much wider at high orders of infection, which makes sense, because we have relatively little data here, as shown in the plots below. It appears as though the risk of an initial malaria infection is lower than infections 2-4 after which the risk sharply drops. The shape of this relationship is robust to excluding low-level infections which may inflate the denominator.

There's a clear positive relationship with age, so I thought that perhaps the humped relationship has to do with protection from complicated malaria early in life. However, the story is not quite so simple.
...
## Testing additional model parameters
...
age_n_inf_nest <- complicated_data %>%
...
```{r, fig.cap=c("Risk of Complicated malaria 0-6 months", "Risk of Complicated malaria 6-12 months")}
#| layout-ncol: 2
#| 
# full_model <- lme4::glmer(comp_num ~ n_infection+I(n_infection^2)+age_cat+(1|id)+log(parsdens), family = "binomial", data=complicated_data)
# 
# visreg(full_model, xvar="n_infection", by="age_cat", trans=exp)
# 

# put together summary data frame 
old_complicated_data <- filter(complicated_data, age>0.5)
old_complicated_df <- as.data.frame(table(old_complicated_data$n_infection, old_complicated_data$complicatedmalaria))
colnames(old_complicated_df) <- c("n_infection", "disease", "complicated_episodes")
...
plot(young_complicated_n_infection)
plot(old_complicated_n_infection)
```
...
```{r, fig.cap=c("Categorical age as mediator", "N_infection as mediator")}
#| fig-width: 7
#| fig-height: 5
#| layout-ncol: 2
plot(med_out2)
