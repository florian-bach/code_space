...
n_infection_cols <- rev(colorspace::sequential_hcl(n=5, palette = "Lajolla"))
#libraries 
library(patchwork)
library(tidyr)
library(ggplot2)
...
As a reminder, the vast majority of antibodies decrease significantly from birth to six months of life, and remain under the limit of detection at 12 months. The following heatmap shows the percentage of measurements that were out of bounds of the limits of detection / quantification (which are almost entirely too low) for each antibody and each timepoint.
...
long_raw_df%>%
  ...
  arrange(antigen)%>%
  mutate(flag_perc= sum(
    !is.na(
      as.numeric(flag)
      )) / n()
    )%>%
  ggplot(., aes(x=timepoint, y=factor(antigen, levels=rev(unique(long_raw_df$antigen))), fill=flag_perc))+
         geom_tile()+
         theme_minimal()+
  theme(axis.text = element_text(size=12))+
         ylab("")+
  xlab("Timepoint")+
         guides(fill = guide_colorbar(title="% of Flagged\nMeasurements", direction = "horizontal", title.position = "top"))+
         viridis::scale_fill_viridis(option="B", labels=scales::percent_format(), breaks = seq(0,1,by=0.25))
```

This limits our ability to track change through time but for a good few antibodies we have a reasonable amount of valid measurements at six and twelve months.

To get a quick overview I decided to do linear regression on each antibody through time. For this fitted linear mixed effects models to predict log-titres by categorical timepoint, including individual identity as a random effect to account for repeated measures and uneven sampling. Regression coefficients are tested using one-sample z tests and p-values adjusted with Benjamini-Hochberg, using an FDR cutoff of 0.1.
...
Of the antibodies with sufficient data, only GEXP significantly increased from 6 months to 12 months, while all decreased from timepoint 1 to timepoint 2.

To see whether there might be combinational trends in the data I decided to run some PCAs on the data. As missing data is not compatible with PCAs, I used the limit of detection / quantification for values out of bounds (which does make the data somewhat synthetic, especially for the measurement at 12 months).
...
wide_combo <- raw_combo_data %>%
  ...
  pivot_wider(names_from = antigen, values_from = log_conc, id_cols = colnames(raw_combo_data)[c(1,2,7:18)])%>%
  #dplyr::select(-flag, -flag_type)%>%
  #distinct()%>%
  dplyr::select(-GST)%>%
  na.omit()
...
There doesn't seem to be any obvious clustering of individuals based on malaria incidence, but let's a have a more detailed look at each individual antibody. Based on the antibody data at birth I modelled the incidence of any malaria, symptomatic malaria, and the conditional probability of symptoms in the first 6 and 12 months of life, separately for each antibody. Incidences were modelled as counts using a negative binomial distribution, conditional probabilities were fit using binomial distributions. After multiple testing correction, five antibodies were predictive of a lower incidence of malaria in the first six months of life: EBA181, HSP40, MSP1, MSP2, Rh2.
...
Based on the antibody concentrations at six months I modelled the incidence of any malaria, symptomatic malaria and the conditional probability of symptoms in months 6-12. I tested whether the incidence of infections in month 0-6 predicted antibody concentrations at 6 months. Indeed, the latter was the only significant result, with SBP1 and HYP2 emerging as markers of exposure.

```{r}
#| label: per-antibody glms six months
#| fig-width: 7
#| fig-height: 7
#| fig-cap: Markers of Exposure
#| fig-align: center
...
Lastly, based on the antibody concentrations at 12 months of life, I modelled the incidence of any malaria, symptomatic malaria and the conditional probability of symptoms in the months 12-18 and 12-24 as well as any malaria in the months 0-12 and 6-12. The only significant interaction was HYP2 where high levels at 12 months of life predicted a higher incidence of symptomatic malaria in moths 12-18, underscoring the the previous finding of HYP2 as a marker of exposure.
...
I thought it was quite curious that GEXP appeared to increase on a population level from 6 to 12 months, but that it did not emerge as a marker of exposure. I looked at the results again, but this apparent paradox seems to be present in the data (shown are GEXP, SEA and GST, the three markers with the lowest adjusted p values for changing between timepoints 2 and 3).
...
cowplot::plot_grid(ggs_through_time, ggs_vs_exsure6_12, ggs_vs_exsure_symp_6_12, ncol = 1)
```
