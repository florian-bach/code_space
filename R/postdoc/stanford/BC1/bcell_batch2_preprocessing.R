...
dir_prepr <- "~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/preprocessed/"
dir_raw <- "~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/"
files <- list.files(dir_raw, pattern="*.fcs", full.names = FALSE, recursive = FALSE)
dir_QC <- "~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/peacoQC/"
comp_dir <- "~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/comp_matrix.csv"
...
channels_of_interest <- rownames(compensation_matrix)

ff_m <- PeacoQC::RemoveMargins(reference_file, channels_of_interest)
ff_c <- flowCore::compensate(ff_m, compensation_matrix)
translist <- estimateLogicle(ff_c, colnames(compensation_matrix), t=262144)
...
# compensation ####
ss_file_paths <- list.files("~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/comp_controls", pattern="*.fcs", full.names = TRUE)
ss_files <- list.files("~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/comp_controls/", pattern="*.fcs", full.names = FALSE)


frames <- lapply(ss_file_paths, function(x)read.FCS(x, transformation = FALSE))
...
comp <- flowStats::spillover(frames,
                  unstained="UNSTAINED",
                  #patt="fluo_",
                  fsc = "FSC-H",
                  ssc = "SSC-H",
...
write.csv(comp, "~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/comp_matrix.csv", quote = FALSE, row.names = TRUE)
...
fcs_file_paths <- list.files("~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/", pattern="*.fcs", full.names = TRUE)
...
for(i in bcell_markers[-3]){
  ...
  ggsave(paste("~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/figures/comp/compensated_", i, "_cd19_biaxial.png", sep = ""), marker_biaxial , height=20, width=40, bg="white")
}


# qc####
try <- read.flowSet(list.files("~/postdoc/stanford/clinical_data/BC1/b_cell_data/raw_batch2/preprocessed/", pattern="*.fcs", full.names = TRUE), truncate_max_range = FALSE)

marker_biaxial <- ggcyto(try, aes(x = `CD19`, y = `CD10`))+
  # ggcyto_par_set(limits = list(x = c(1,10e5), y= c(1,10e5)))+
  scale_x_logicle()+
  scale_y_logicle()+
  geom_hex(bins=512)+
...
ggsave(paste(dir_prepr, "actually_compensated.png", sep=""), marker_biaxial, bg="white", height=16, width=24)
