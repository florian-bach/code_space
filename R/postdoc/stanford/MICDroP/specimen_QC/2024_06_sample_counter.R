...
ggsave("~/postdoc/stanford/clinical_data/MICDROP/specimen_QC/2024_06/collection_rate_plot.png", collection_rate_plot, bg="white", dpi=444, width=8, height=4)
...
collection_rate_plot <-   sample_counts%>%
  ...
  geom_bar(stat="identity")+
  facet_grid(~Specimen_Type)+
  geom_text(aes(y=1.07, label=paste(Number_of_Participants, sep="")), size=3)+
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0,1,0.2), limits=c(0,1.11))+
  scale_fill_manual(values = colorspace::sequential_hcl(n=7, "RdPu")[1:6])+
  xlab("Timepoint (weeks)")+
  ylab("Collection Rate")+
...
ggsave("~/postdoc/stanford/clinical_data/MICDROP/specimen_QC/2024_06/collection_rate_plot2.png", collection_rate_plot, bg="white", dpi=444, width=8, height=4)
