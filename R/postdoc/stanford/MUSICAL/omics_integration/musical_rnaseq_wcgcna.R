...
expression.data <- as.data.frame(expression_data%>%
...
write.csv(traitRows, "~/postdoc/stanford/plasma_analytes/MUSICAL/combo/wgcna_RNAseq_MEs.csv", row.names = F)
...
curve_plot1 <- long_kme_p%>%
  ...
  filter(network %in% c("MEsalmon", "MEyellow", "MEblack", "MEmagenta"))%>%
  # mutate(network=case_match(network, "MEturquoise"~"fever",
  #                           "MEred"~"parasite density",
  #                           "MEmagenta"~"parasite clearance", 
  #                           "MEbrown"~"combined fever and parasites"))%>%
