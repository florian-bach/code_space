...
flo_plate <- read.csv("~/postdoc/stanford/cytometry/attune/FB03/2024_FB03_fbs_ab_no_serum 2/flo/flo_activation_freqs.csv")
julio_plate <- read.csv("~/postdoc/stanford/cytometry/attune/FB03/2024_FB03_fbs_ab_no_serum 2/julio/julio_activation_freqs.csv")
...
flo_long_data <- {flo_plate %>%
  ...
  mutate(cell_pop=gsub("WBC.Singlets.CD3..", "", .$cell_pop))%>%
...
julio_long_data <- {julio_plate %>%
  pivot_longer(cols=colnames(julio_plate)[2:(ncol(julio_plate)-1)], names_to="cell_pop", values_to="freq")%>%
  mutate("sample"=X)%>%
  select(sample, cell_pop, freq, -X.1)%>%
  mutate(cell_pop=gsub("WBC.Singlets.CD3..", "", .$cell_pop))%>%
  mutate("lineage"=case_when(
  ...
  mutate("person"="Julio", "origin"="USA")%>%
  filter(quadrant!="Q4")%>%
  group_by(sample, lineage, marker_combo, stim,  serum, id)%>%
  add_summary_rows(quadrant="sum(Q)",
                   freq=sum(freq))
...
long_data <- rbind(julio_long_data, flo_long_data)
...
ggsave("~/postdoc/stanford/cytometry/attune/FB03/figures/cd4_cd4neg_summary_plot.png", cd4_cd4neg_summary_plot, width=8*1.5, height=8*1.5, bg="white")
