...
flo_plate <- read.csv("~/postdoc/stanford/cytometry/attune/FB05/pop_freqs.csv")
...
flo_long_data <- {flo_plate %>%
    ...
    mutate("serum"=case_when(
      ...
      grepl("9.fcs$", .$sample) ~ "US",
      grepl("10.fcs$", .$sample) ~ "US",
    ))%>%
    mutate("id"=case_when(
      grepl("^B", .$sample) ~ "415",
      grepl("^C", .$sample) ~ "420",
...
ggsave("~/postdoc/stanford/cytometry/attune/FB05/figures/cd4_cd4neg_summary_plot.png", cd4_cd4neg_summary_plot, width=8*1.5, height=10*1.5, bg="white")
...
ggsave("~/postdoc/stanford/cytometry/attune/FB05/figures/cd4_plot.png", cd4_plot, width=6, height=4, bg="white")
