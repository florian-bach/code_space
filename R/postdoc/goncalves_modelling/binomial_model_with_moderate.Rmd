...
Given each of these probabilities, we ask how often one would observe severe disease, if X number of malaria cases were recorded, where X is the number of observations for each order of infection. This process is repeated 10,000 times to obtain means and confidence intervals.

The plot below shows the distribution of simulated results as violin plots. The area of two standard deviations (95% confidence interval) is shaded light grey with the mean in black. The actual values observed in the paper are shown in red for severe cases and blue for moderate/severe cases. For severe cases, the observed value is within the 95% confidence interval, but only just- moderate/severe cases are outwith the 95% confidence interval in first infection regardless of how the mean/median probability was calculated.
