...
As a reminder, on the left are the fractions we're modelling. To deal with missing data at infections 12 & 13 I used weighted linear regression of non-`NA`, log-transformed case counts to model the number of cases at each infection and imputed the rounded values at 12 & 13.
...
n_infection_decay <- lm(log(Supp_Infections)~N_Infection, data=goncalves_data[2:15,], weights = Supp_Infections)

prd <- data.frame(N_Infection = seq(from = 1, to = 14))
err <- predict(n_infection_decay, newdata = prd, se.fit = TRUE)
prd$modelled_cases <- exp(err$fit)
prd$round_model <- round(prd$modelled_cases)

goncalves_data$Modelled_Supp_Cases <- ifelse(is.na(goncalves_data$Supp_Infections), prd$round_model, goncalves_data$Supp_Infections)

goncalves_data$Imputed <- ifelse(is.na(goncalves_data$Supp_Infections), TRUE, FALSE)
...
supp_case_decay <- ggplot()+
  geom_point(data=goncalves_plot_data, aes(x=N_Infection, y=Modelled_Supp_Cases, colour=Imputed))+
geom_line(data = prd, aes(x = N_Infection, y=prd$modelled_cases), colour="purple", linetype="dashed")+
  ylab("\n\nNumber of Cases\n")+
...
brute_force <- data.frame(matrix(ncol=9))

colnames(brute_force) <- c("P1", "SE(P1)", "alpha_1", "SE(alpha_1)", 	"alpha_2", 	"SE(alpha_2)", "j", "logL", "AIC")
...
for(number in 2:8){
  ...
  brute_force[number-1,] <- new_entry
}
...
fit <- glm(Severe_Moderate/Modelled_Supp_Cases~N_Infection, weights = Supp_Infections, family="binomial", data=goncalves_plot_data)
...
# step_wise_models <- data.frame(matrix(nrow=6))
# 
...
knitr::kable(brute_force, caption="Stepwise Models") %>%
  kableExtra::kable_styling(full_width = TRUE)


```
I've tweaked the stepwise models a little bit, so now they can be evaluated for any value of j, however adding the extra 3 data points now means that some resulting model parameters are estimated to have negative variance which means their standard error cannot be evaluated. I've improved this by setting upper and lower bounds for each variable but currently this is the best I can do.

Based on AIC & log likelihood, exponential decay provides the best fit for this data. The corresponding model is shown below on the left, next to the logistic regression model suggested by `stats::glm()`.
...
glm_model <- ggplot(goncalves_plot_data, aes(x = N_Infection, y=Severe_Moderate/Modelled_Supp_Cases))+
  ylab("\n\nRisk of Severe / Moderate Disease\n")+
...
hand_model <- ggplot(goncalves_plot_data, aes(x = N_Infection, y=Severe_Moderate/Modelled_Supp_Cases))+
  ylab("\n\nRisk of Severe / Moderate Disease\n")+
  scale_y_continuous(label=scales::percent, limits=c(0, 0.15))+
  scale_x_continuous(limits=range(0, nrow(goncalves_plot_data)-1), breaks=seq(0, nrow(goncalves_plot_data)-1), expand = expansion(add = c(0.2,0.2)))+
  geom_point(colour="blue")+
